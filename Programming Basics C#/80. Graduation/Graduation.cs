    ...
    internal class Graduation
    {
        static void Main(string[] args)
        {
            ...
            while (grade <= 12)
            {
                ...
                if (a >= 4.00) { total += a; grade++; }
                else { if (fail < 1) { fail++; } else { Console.WriteLine($"{name} has been excluded at {grade} grade"); break; } }
            }
            if (grade > 12 ) { Console.WriteLine($"{name} graduated. Average grade: {total/12:f2}"); }
        }
    }
}
